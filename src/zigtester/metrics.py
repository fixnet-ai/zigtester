...
_BENCH_PATTERNS: list[tuple[str, str]] = [
    ...
    (r"REQ_S=([0-9.]+)", "req_s"),
    (r"FAILED=([0-9]+)", "failed"),
    # bench_long（长时持续压测，test_engine.py --long）单行 KEY=VALUE:
    # 资源指标(rss_growth/fd_growth)由框架 ResourceMonitor 注入,脚本不输出
    (r"DURATION_S=([0-9.]+)", "duration_s"),
    (r"OK_REQ=([0-9]+)", "success_count"),
    (r"ERRORS=([0-9]+)", "error_count"),
    (r"ERROR_RATE=([0-9.]+)", "error_rate"),
    # 长时压测中文格式（zigbox test_stress.py，经此不依赖 yaml custom 也能入库）:
