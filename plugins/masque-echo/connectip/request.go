...
// requestProtocol 是服务器接受的 CONNECT :protocol 值。
// 默认 "cf-connect-ip" 对齐 Cloudflare WARP 非标形态（RFC 9484 标准为 "connect-ip"）。
var requestProtocol = "cf-connect-ip"

// SetRequestProtocol 覆盖协议名（如测试标准 connect-ip 时设回 "connect-ip"）。
func SetRequestProtocol(p string) { requestProtocol = p }
