...
// AllowAnyDestination 控制服务器是否校验收到的 IP 包 dst 地址。
// Cloudflare WARP 服务器不 AdvertiseRoute（localRoutes 空）但仍收任意 dst 包（真转发），
// 故默认 true 跳过 dst 校验，对齐 CF 服务器端行为。设为 false 恢复 RFC 9484 标准校验。
var AllowAnyDestination = true
...
func (c *Conn) handleIncomingProxiedPacket(data []byte) error {
	...
	// Cloudflare WARP 服务器不 AdvertiseRoute（localRoutes 空）仍收任意 dst 包（真转发），
	// AllowAnyDestination=true 时跳过 dst 校验，对齐 CF 服务器端行为。
